 ...
 * saturates.
 *
 * <p>The math, motor names, and motor-direction choices here match the
 * version of {@code BasicTeleOp} on master (Y-left field frame,
 * CCW-positive heading from Pinpoint). BasicTeleOp is left alone; this
 * helper is what the new experimental OpModes will use, so all of them get
 * identical drive behavior without duplicating the math.
 */
public class MecanumDrive {
    ...
    /**
     * Rotate a field-frame command into the robot frame and apply it.
     *
     * <p>Convention: heading is CCW-positive from field +x, and the field
     * frame is Y-left (field +Y is to the driver's left). The stick input
     * however names the driver's RIGHT as positive, so we take
     * {@code fieldRight} (driver's right) instead of a "field +Y" value; the
     * math below accounts for that sign.
     *
     ...
     * @param yaw           +1 = counter-clockwise (robot frame; yaw is the
     *                      same in both frames)
     * @param headingRad    the robot's current heading in radians (CCW from
     *                      field +x, as Pinpoint reports it on this robot)
     */
